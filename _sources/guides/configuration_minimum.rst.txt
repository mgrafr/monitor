...
.. note::
   ...
   **TEMPO_DEVICES_MQTT**
   ...
   - Inconvénients(facilement surmontables) : Installation d'un serveur MQTT, de Paho , création d'un script pour envoyer les données depuis DZ ; le client MQTT dans Javascript est déjà installé, il suffit de mettre en service MQTT
...
1.1.3.1 Solution temps réel MQTT
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
