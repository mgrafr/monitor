...
echo '<script>text1="";</script>';
			  ...
			  if ($tt<time()) {$tt=0;echo "<p id='mp2' >temps pwd dépassé - </p>";
					if (DECOUVERTE==false){echo "<script>document.getElementById('d_btn_al').style.display = 'block';</script>";}}
   			  if ($tt==0){
				  echo "<script>text1='pwd:absent';document.getElementById('d_btn_a').style.display = 'block';";
				  if (DECOUVERTE==false){echo "document.getElementById('d_btn_al').style.display = 'block';";}
				echo "</script>";}
			  else {echo "<script>
			  document.getElementById('info_admin').style.display = 'none';
			  document.getElementById('d_btn_a').style.display = 'none';";
			  if (DECOUVERTE==false){echo "document.getElementById('d_btn_al').style.display = 'none';";}
			  echo "</script>";}
				if (DECOUVERTE==false){  echo "<script>
