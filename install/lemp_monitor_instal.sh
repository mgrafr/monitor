...
exitstatus=$?
if [ $exitstatus = 0 ]; then
msg_info "Utlisateur enregistré : "$maria_name
else
maria_name=monitor
msg_info "Par défaut, utlisateur enregistré : "$maria_name
fi 
adduser $maria_name
usermod -aG sudo $maria_name
msg_info "Utilisateur "$maria_name "enregistré et ajouté au groupe SUDO"
...
msg_info "serveur enregistré:" $server_name
#server_name = "monitor"
...
msg_info "LEMP : Debut de l installation"
msg_info "mmaj debian ,installation de sudo curl git pip"
$STD apt-get update 
$STD apt-get upgrade
echo -e "${GN} \e[1;92m Debian a ete mis à jour.\e[0m"
sleep 3
...
echo -e "${GN} \e[1;92m Debut installation de Maria DB.\e[0m"
sleep 3
...
exitstatus=$?
if [ $exitstatus = 0 ]; then
 msg_info "Mot de passe enregistré"
fi
...
exitstatus=$?
if [ $exitstatus = 0 ]; then
 msg_info "Mot de passe root pour $maria_name enregistré"
fi
msg_info "securisation de mariaDB"
...
echo -e "${GN} \e[1;92m NGINX a été installé.\e[0m"
sleep 3
...
ufw allow https
ufw enable
echo -e "${GN} \e[1;92m Le pare-feu a été installé.\e[0m"
...
echo -e "${GN} \e[1;92m SSHPASS a été installé.\e[0m"
sleep 2
...
apt-get update
echo -e "${GN} \e[1;92m Dépendances installées.\e[0m"
...
echo -e "${GN} \e[1;92m PHP8.4 installé.\e[0m"
sleep 3
...
echo -e "${GN} \e[1;92m phpMyAdmin installé.\e[0m"
